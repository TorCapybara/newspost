...
int main(int argc, char **argv) {
	...
	/* initialize the data */
	main_data.subject[0] = '\0';
	main_data.newsgroup[0] = '\0';
	main_data.from[0] = '\0';
	main_data.organization[0] = '\0';
	main_data.address[0] = '\0';
	main_data.port = 119;
	main_data.user[0] = '\0';
	main_data.password[0] = '\0';
	main_data.lines = 7500;
	main_data.prefix[0] = '\0';
	main_data.yenc = FALSE;
	main_data.sfv[0] = '\0';
	main_data.par[0] = '\0';
	main_data.parnum = 0;
	main_data.filesperpar = 10;
	main_data.reference[0] = '\0';
	main_data.filenumber = FALSE;
	main_data.tmpdir[0] = '\0';
	main_data.noarchive = TRUE;
	main_data.followupto[0] = '\0';
	main_data.replyto[0] = '\0';
	main_data.name[0] = '\0';
	...
	if ((main_data.sfv[0] != '\0')
	   || (main_data.par[0] != '\0')
	   || (temporary_prefix == TRUE)
	   || ((main_data.text == TRUE) && (file_list == NULL)) ) {
		if (main_data.tmpdir[0] == '\0')
			strcpy(main_data.tmpdir, P_tmpdir);
		strcat(main_data.tmpdir, "/newspost-XXXXXX");
		fd = mkstemp(main_data.tmpdir);
		close(fd);
		unlink(main_data.tmpdir);
		if (mkdir(main_data.tmpdir, S_IRWXU) == -1) {
			ui_tmpdir_create_failed(main_data.tmpdir, errno);
			retval =  FAILED_TO_CREATE_TMPFILES;
		}
		else { 
			...
			if ((main_data.text == TRUE) && (file_list == NULL)) {
				file_data = malloc(sizeof(file_entry));
				snprintf(file_data->filename, STRING_BUFSIZE,
					 "%s/message", main_data.tmpdir);
				snprintf(command, STRING_BUFSIZE, "%s %s",
					 (EDITOR != NULL) ? EDITOR : "vi",
					 file_data->filename);
				system(command);
				
				fp = fopen(file_data->filename, "rb");
				if (fp != NULL) {
					fclose(fp);
					chmod(file_data->filename,
						S_IRUSR | S_IWUSR);
					stat(file_data->filename,
						&file_data->fileinfo);
				 ...
				 (retval == NORMAL) &&
				 (main_data.text == FALSE)) {
				snprintf(main_data.prefix, STRING_BUFSIZE,
					 "%s/prefix", main_data.tmpdir);
				snprintf(command, STRING_BUFSIZE, "%s %s",
					 (EDITOR != NULL) ? EDITOR : "vi",
					 main_data.prefix);
				system(command);
				
				fp = fopen(main_data.prefix, "rb");
				if (fp != NULL) {
					fclose(fp);
					chmod(main_data.prefix,
						S_IRUSR|S_IWUSR);
				}
				else {
					fprintf(stderr,
	...
	if (TRUE == temporary_prefix)
		unlink(main_data.prefix);
