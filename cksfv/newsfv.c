...
void newsfv(SList *file_list, newspost_data *np_data)
{
	...
	fn = np_data->sfv;
	fn += (strlen(np_data->sfv) - 4);
	if (strncasecmp(fn, ".sfv", 4) != 0) {
		fn += 4;
		sprintf(fn, ".sfv");
	}

	sprintf(tmpstring, "%s/%s", np_data->tmpdir, np_data->sfv);
	strncpy(np_data->sfv, tmpstring, STRING_BUFSIZE);

	sfvfile = fopen(np_data->sfv, "wb");
	if (sfvfile == 0) {
		ui_sfv_gen_error(np_data->sfv, 0);
		return;
	}
	...
	while (file_list != NULL) {
		...
		fprintf(sfvfile, "%s %08X\r\n", basename(fn), data->crc);
		
		file_list = slist_next(file_list);
	}

	fclose(sfvfile);
	chmod(np_data->sfv, S_IRUSR | S_IWUSR);

	ui_sfv_gen_done(np_data->sfv);
}
